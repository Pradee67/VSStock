...
namespace ProjectReact.Controllers
{
    ...
    public class LoginController : ControllerBase
    {
        ...
        public LoginController(UserManager<IdentityUser> um, RoleManager<IdentityRole> rm, IConfiguration config)
        {
            _usermanger = um;
            _rolemanager = rm;
            _config = config;
        ...
        {
            var userExists = await _usermanger.FindByNameAsync(model.Email);
            if (userExists != null)
                return StatusCode(StatusCodes.Status500InternalServerError, new Response { Status = "Error", Message = "User already exists!" });
